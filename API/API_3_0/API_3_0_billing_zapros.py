...
def balance(service = None):
    ...
    if service == None:
        ...
        service_name = str(service['SubSystem']) + '.' + str(service['Services'][0])
        Count_answer = r.json()['Count'][service_name]
        Overdraft_answer = r.json()['Overdraft'][service_name]
        #print(str(service_name) + ' Count - ' + str(Count_answer))
        #print(str(service_name) + ' Overdraft - ' + str(Overdraft_answer))
        return Count_answer, Overdraft_answer

def checkAllServices():
    for service in SERVICES:
        service_name = str(service['SubSystem']) + '.' + str(service['Services'][0])
        first_Count_balance, first_Overdraft_balance = balance(service)
        if first_Count_balance != 0 or first_Overdraft_balance != 0:
            request(service)
            second_Count_balance, second_Overdraft_balance = balance(service)
            if first_Count_balance == 0:
                print(service_name + ' Overdraft balanse - ZERO')
            if first_Count_balance - second_Count_balance == 1:
                print(service_name + ' Count balanse - OK')
            if first_Count_balance - second_Count_balance != 1 and first_Count_balance != 0:
                print(service_name + ' Count balanse - FAIL')
                Fails.append(service_name + ' Count balanse - FAIL')

            if first_Overdraft_balance == 0:
                print(service_name + ' Overdraft balanse - ZERO')
            if first_Overdraft_balance - second_Overdraft_balance == 1:
                print(service_name + ' Overdraft balanse - OK')
            if first_Overdraft_balance - second_Overdraft_balance != 1 and first_Overdraft_balance != 0:
                print(service_name + ' Overdraft balanse - FAIL')
                Fails.append(service_name + ' Overdraft balanse - FAIL')

        else:
            print(service_name + ' Count and Overdraft balanses - ZERO')

    if Fails != []:
        print('\n' + '------------------Все плохо, в биллинке запросов есть ошибки:')
        for fail in Fails:
            print(fail)
    else:
        print('\n' + '------------------Все в порядке, биллинг запросов работает!')

def checkOneService(service):
    service_name = str(service['SubSystem']) + '.' + str(service['Services'][0])
    first_Count_balance, first_Overdraft_balance = balance(service)
    if first_Count_balance != 0 or first_Overdraft_balance != 0:
        request(service)
