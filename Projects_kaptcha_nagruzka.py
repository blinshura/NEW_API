...
sllep_time = 1  # задеражка запуска потоков
sleep_tread = 2 # задеражка запуска потоков

connect_timeout = 100000   #
timeout = 300000  # port 80: Timed out
printing = []
ITER = []
Exceptions = []
Exceptionse1 = []
        ...
        if id != 0:
            try:
                ...
                while ok != 'OK' + okt:
                    ...
                    go = gg.go('95.31.1.216/res.php?id=' + str(id) + '&key=' + key + '&action=get')
                    mxmlBODY = (go.body)
...
try:
    while thread < THREADS:
        ...
        p = threading.Thread(target=login, name=t, args=[REQUESTS, t]) # args - количество запросов
        p.start()
        sleep(5)
...
print('Запросов в потоке:' + str(REQUESTS))
print('Должно быть отправлено картинок: ' + str(REQUESTS * len(working_threds)))
print('Отправлено картинок: ' + str(len(ITER)))
print('Потеряно картинок: ' + str(REQUESTS * len(working_threds) - len(ITER)))
total_time = b - a
